...
def UpdateCBCE(recvQueue):
    ...
    def train(tuple2):
        ...
        ##if class label exist
            ...
            if len(potentialCF[tuple[0]][k]) >= 5:
                ...
                finalCf[tuple[0]]['centroid'] = array(finalCf[tuple[0]]['linearSum']) / float(
                    len(finalCf[tuple[0]]['microCent']))
                finalCf[tuple[0]]['radius'] = sum(
                    array([finalCf[tuple[0]]['squareSum'], (-1) * array(finalCf[tuple[0]]['linearSum']) ** 2], 0))
  ...
  ##  for t in tuple:
    ...
    def test(tuple2):
        del (record[0])
        for key in ensemble:
            prob = calculateProbability(record, mean(ensemble[key]['positive']), stdev(ensemble[key]['positive'])) * \
                   ensemble[key]['priorProbab']
